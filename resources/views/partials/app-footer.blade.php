<footer class="modern-footer">
  <div class="footer-container">
    ...
    <div class="footer-nav">
      <h4>{{ $quickAccessTitle ?? 'Accès rapide' }}</h4>
      @if(isset($quickAccess) && is_iterable($quickAccess))
      @foreach($quickAccess as $item)
        <a href="{{ $item['url'] ?? '#' }}"><i class="{{ $item['icon'] ?? 'fas fa-circle' }}"></i> {{ $item['label'] }}</a>
      @endforeach
      @endif
    </div>

    <div class="footer-account">
      <h4>{{ $usefulTitle ?? 'Informations' }}</h4>
      @if(isset($useful) && is_iterable($useful))
